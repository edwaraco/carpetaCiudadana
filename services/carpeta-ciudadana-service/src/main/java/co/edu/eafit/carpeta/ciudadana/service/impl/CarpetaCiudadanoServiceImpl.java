...
public class CarpetaCiudadanoServiceImpl implements CarpetaCiudadanoService {
  ...
  public CarpetaCiudadano crearCarpeta(CrearCarpetaRequest request) {
    ...
    carpetaRepository
        .findByPropietarioCedula(request.cedula())
        .ifPresent(
            carpeta -> {
  ...
  public Documento subirDocumento(SubirDocumentoConArchivoRequest request, MultipartFile archivo) {
    ...
    carpetaRepository
        .findById(request.carpetaId())
        .orElseThrow(
            () -> new ResourceNotFoundException("Carpeta", "carpetaId", request.carpetaId()));
    ...
    try {
      ...
      log.error(
          "Error subiendo documento a carpeta {}: {}", request.carpetaId(), e.getMessage(), e);
